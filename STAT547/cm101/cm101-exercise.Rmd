...
square <- function(x) {
  y <- x^2
  return(y) #return will stop a function
  5 # since there is a return, it will skip remaining lines
}
...
```{r}
expect_equal(0.1 + 0.2, 0.3) #if test passes, then it does not return anything
...
mmm2 <- function(x) {
    if (!is.numeric(x)) {
        stop(paste("Expecting to be numeric. You gave me",
                   typeof(x)))
    }
    max(x) - min(x)
}
